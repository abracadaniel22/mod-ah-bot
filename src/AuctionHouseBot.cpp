...
void AuctionHouseBot::addNewAuctions(Player* AHBplayer, AHBConfig *config)
{
    ...
    if (debug_Out)
        LOG_ERROR("module", "AHSeller: House ID: {}. Current auction count: {}. Adding {} auctions", config->GetAHID(), auctions, items);
...
void AuctionHouseBot::addNewAuctionBuyerBotBid(Player* AHBplayer, AHBConfig *config)
{
    ...
    for (uint32 count = 1; count <= config->GetBidsPerInterval(); ++count)
    {
        ...
        calculateItemValueForBuyer(prototype, willingToSpendPerItemPrice);
